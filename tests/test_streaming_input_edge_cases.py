...
@pytest.mark.parametrize(
    "factory",
    [
        lambda: (chunk for chunk in [bytearray(b"<root/>")]),
        lambda: (chunk for chunk in [memoryview(b"<root/>")]),
        lambda: (chunk for chunk in [BytesSubclass(b"<root/>")]),
    ],
)
def test_generator_bytes_like_variants(factory):
