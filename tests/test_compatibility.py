...
class TestForceList:
    ...
    @pytest.mark.parametrize("force_list_value,expected_type", [
        (True, list),
        (False, str),
        (('server',), list),
        (['server'], list),
    ...
    @pytest.mark.parametrize("xml,force_list,expected_structure", [
        # Single element should become list
        ('<servers><server>test</server></servers>', ('server',),
         {'servers': {'server': ['test']}}),

        # Multiple elements should be list
        ('<servers><server>1</server><server>2</server></servers>', ('server',),
         {'servers': {'server': ['1', '2']}}),
