...
class AccurateBenchmark:
    ...
    def measure_function(self, func: Callable, avg: float, *args, **kwargs) -> tuple[List[float], int]:
    ...
    def compare_functions(self, test_name: str, func1: Callable, func2: Callable,
                         name1: str, name2: str, data_size_bytes: int, *args, **kwargs) -> Dict[str, Any]:
        ...
        print(f"\n  📈 Results:")
        print(f"    {name1:>15}: {format_time(stats1['mean']):>10} ± {format_time(stats1['stddev']):>8}")
        print(f"    {name2:>15}: {format_time(stats2['mean']):>10} ± {format_time(stats2['stddev']):>8}")
    ...
    def print_detailed_stats(self, name: str, stats: Dict[str, float], data_size_bytes: int = 0) -> None:
...
def run_accurate_benchmarks():
    ...
    for test_name, xml_data in test_data.items():
        ...
        parse_result = benchmark.compare_functions(
            f"Parse - {test_name}",
            xmltodict.parse, xmltodict_rs.parse,
            "xmltodict", "xmltodict_rs",
            size_bytes, xml_data
        )

        # Detailed parse statistics
        benchmark.print_detailed_stats("xmltodict (parse)", parse_result['func1']['stats'], size_bytes)
        benchmark.print_detailed_stats("xmltodict_rs (parse)", parse_result['func2']['stats'], size_bytes)
        ...
        unparse_result = benchmark.compare_functions(
            f"Unparse - {test_name}",
            xmltodict.unparse, xmltodict_rs.unparse,
            "xmltodict", "xmltodict_rs",
            size_bytes, parsed_data
        )

        # Detailed unparse statistics
        benchmark.print_detailed_stats("xmltodict (unparse)", unparse_result['func1']['stats'], size_bytes)
        benchmark.print_detailed_stats("xmltodict_rs (unparse)", unparse_result['func2']['stats'], size_bytes)
    ...
    for result in all_results:
        ...
        print(f"{test_name:<20} {size:<10} {parse_speedup:>8.2f}x {unparse_speedup:>8.2f}x {parse_throughput:>13} {unparse_throughput:>15}")
