...
const IO_PYERR_PREFIX: &str = "PYERR|";
...
fn pyerr_to_io(err: &PyErr) -> io::Error {
    Python::attach(|py| {
        ...
        io::Error::other(format!("{IO_PYERR_PREFIX}{err_type}|{err_msg}"))
    })
}

fn pyerr_from_io(err: &io::Error) -> Option<PyErr> {
    let msg = err.to_string();
    let rest = msg.strip_prefix(IO_PYERR_PREFIX)?;
    let mut parts = rest.splitn(2, '|');
    let err_type = parts.next()?;
    let err_msg = parts.next().unwrap_or("");

    Some(match err_type {
        "TypeError" => PyErr::new::<pyo3::exceptions::PyTypeError, _>(err_msg.to_string()),
        "ValueError" => PyErr::new::<pyo3::exceptions::PyValueError, _>(err_msg.to_string()),
        "RuntimeError" => PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(err_msg.to_string()),
        "StopIteration" => PyErr::new::<pyo3::exceptions::PyStopIteration, _>(err_msg.to_string()),
        _ => PyErr::new::<pyo3::exceptions::PyException, _>(err_msg.to_string()),
    })
}
            ...
            let bytes = if let Ok(chunk_bytes) = chunk.downcast::<PyBytes>() {
                ...
                if let Some(bytes_ref) = self.bytearray_buffer.as_deref() {
                    ...
                    return Err(io::Error::other(format!(
                        "{IO_PYERR_PREFIX}TypeError|read() did not return a bytes object (type=bytearray)"
                    )));
                }
            } else {
                let type_name = chunk
                ...
                return Err(io::Error::other(format!(
                    "{IO_PYERR_PREFIX}TypeError|read() did not return a bytes object (type={type_name})"
                )));
            };
...
impl PyGeneratorRead {
    ...
    fn next_non_empty_chunk<'py>(
        ...
        while !self.done {
            ...
            if chunk.is_none() {
                return Err(io::Error::other(format!(
                    "{IO_PYERR_PREFIX}TypeError|a bytes-like object is required, not 'NoneType'"
                )));
            }
            ...
            if let Ok(chunk_memview) = chunk.downcast::<PyMemoryView>() {
                ...
                let bytes = bytes_obj
                    .downcast::<PyBytes>()
                    .map_err(|_| io::Error::other(format!("{IO_PYERR_PREFIX}TypeError|a bytes-like object is required, not 'memoryview'")))?;
        ...
        Python::attach(|py| {
            ...
            let bytes = if let Ok(chunk_bytes) = chunk.downcast::<PyBytes>() {
                ...
                if let Some(bytes_ref) = self.bytearray_buffer.as_deref() {
                    ...
                    return Err(io::Error::other(format!(
                        "{IO_PYERR_PREFIX}TypeError|a bytes-like object is required, not 'bytearray'"
                    )));
                }
            } else {
                let type_name = chunk
                    ...
                    .unwrap_or_else(|_| "unknown".to_string());
                return Err(io::Error::other(format!(
                    "{IO_PYERR_PREFIX}TypeError|a bytes-like object is required, not '{type_name}'"
                )));
            };
